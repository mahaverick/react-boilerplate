...
test.beforeAll(async () => {
  ...
  if (!served?.ok) {
    throw new Error(
      `No app at ${APP_ORIGIN}. Build and run the image: docker build -t react-boilerplate:e2e . && docker run -d --name rb-e2e -p 8088:8080 --add-host=api:host-gateway react-boilerplate:e2e`
    )
  }
})
