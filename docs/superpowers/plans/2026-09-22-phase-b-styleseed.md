...
| 6 | `ss-verify` out of scope, no Playwright, `package.json` untouched | Playwright 1.63.0 installed; gate run at 1440×900 and 390×844 | Reversed by the user after the rest landed. The gate found a defect code review had only inferred, and verified `min-w-2xl`'s horizontal scroll, closing open-items §4. Visual score 85 against a code score of 93 |
| — | (not in the plan) | The four visual findings fixed; a Playwright e2e suite added; open-items §1 executed against a live backend | Follow-on work the user asked for after the plan was complete. Notable: open-items §1.3 was **mis-stated** — `X-Forwarded-Proto` does not drive the `secure` flag, `NODE_ENV` does — and the SSE reconnect proved unverifiable through the Vite dev proxy, which does not propagate the upstream close |
